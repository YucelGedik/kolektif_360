...
clamped 24 even though the read itself was correct). Ranges here are kept
generously above the known real defaults precisely to avoid repeating that;
they are still not official mechanical limits and should be tightened once
the PLC engineer confirms final ones.
...
PARAMETER_SPECS: list[ParameterSpec] = [
    ParameterSpec("lr_x_cut_velocity", "X Kesim Hızı", "mm/s", 175.0, 1.0, 500.0),
    ParameterSpec("lr_x_cut_acc_dec", "X Kesim Acc/Dec", "mm/s²", 500.0, 1.0, 10000.0),
    ParameterSpec("lr_x_return_velocity", "X Dönüş Hızı", "mm/s", 400.0, 1.0, 800.0),
    ...
    ParameterSpec("lr_y_move_velocity", "Y Pozisyonlama Hızı", "mm/s", 10.0, 1.0, 200.0),
    ParameterSpec("lr_y_move_acc_dec", "Y Pozisyonlama Acc/Dec", "mm/s²", 100.0, 1.0, 10000.0),
    ParameterSpec("lr_y_max_velocity", "Y Follow Maks. Hızı", "mm/s", 5.0, 1.0, 200.0),
    ParameterSpec("lr_x_jog_velocity", "X Jog Hızı", "mm/s", 50.0, 1.0, 200.0),
    ParameterSpec("lr_y_jog_velocity", "Y Jog Hızı", "mm/s", 5.0, 1.0, 100.0),
